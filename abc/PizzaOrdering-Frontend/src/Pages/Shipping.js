...
const Shipping = () => {
    ...
    const getAddressesByUser = async () => {
        await axios.get(`${hostName}/addressbyuser/${userId}`).then((res) => {
    ...
    const loadCart = async () => (
        ...
        await axios.get(`${hostName}/cart/${userId}`).then((res) => {
            setCart(res.data)
    ...
    const loadItems = async () => {
        await axios.get(`${hostName}/cartitems/${cart.id}`).then((res) => {
    ...
    const deleteAddress = async (id) => {
        try {
            await axios.delete(`${hostName}/address/${id}`).then((res) => {
                setAllAdresses(res.data)
                        ...
                        <div className="add-sec-area">
                            {
                                allAdress.length > 0 ? (allAdresses.map((address, i) => (
                                    ...
                                    <div onClick={() => handleSelectAddress(address, i)} 
                                    className={`og-add ${selected === i && 'selected'}`} key={address.id}>
