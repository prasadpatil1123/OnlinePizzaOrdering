...
public class ShoppingCartServiceImpl implements ShoppingCartService {
	...
	// Cart(addCartToUser, addToCart, udateCart, removeFromCart,DeleteCart,
	// getCartByUserID)--------------------------------------------------------------
...
//		ShoppingCart updatedCart = this.updateCart(cart);
	...
	public Order checkout(long user_id, long address_id, String paymentType, double discount, double deliveryPrice,
			double taxAmount) {
		/*
		 * Step 1. Get list of cart items step 2. create new order step 3. create new
		 * order item for each order - in loop step 4. copy each cart item to order
		 * items - in loop Step 5. delete cart item - in loop step 5. copy additional
		 * info (total price etc) to orders step 6. update the cart (total price, count
		 * etc) step 7. save updated cart to DB step 8. save & return order
		 */
